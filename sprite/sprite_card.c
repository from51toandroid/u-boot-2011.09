...
{     
    ...
    partsize_by_byte = partsize_by_byte<<9;
    //打开分区镜像
    imgitemhd = Img_OpenItem(imghd, "RFSFAT16", (char *)part_info->dl_filename);
    if( !imgitemhd ){
        printf("sunxi sprite error:open part %s failed\n", part_info->dl_filename);
    ...
    if( partdata_format != ANDROID_FORMAT_DETECT ){
        ...
        while( tmp_partdata_by_bytes >= SPRITE_CARD_ONCE_DATA_DEAL ){
               ...
               if( sunxi_flash_read(tmp_imgfile_start, SPRITE_CARD_ONCE_SECTOR_DEAL, down_buffer) != 
               ...
               //写入flash
               if( sunxi_sprite_write(tmp_partstart_by_sector, SPRITE_CARD_ONCE_SECTOR_DEAL, down_buffer) != 
                   SPRITE_CARD_ONCE_SECTOR_DEAL ){
