...
static int __usb_get_descriptor(struct usb_device_request *req, uchar *buffer)
{
    ...
    switch( req->wValue >> 8 ){
      ...
      case USB_DT_CONFIG:		
        ...
        //最大电流500ms(0xfa * 2)
        bytes_remaining = bytes_remaining - config_dscrptr->bLength;
        //interface
...
static int __sunxi_usb_efex_op_cmd(u8 *cmd_buffer)
{
    ...
    switch( cmd->app_cmd ){
      case APP_LAYER_COMMEN_CMD_VERIFY_DEV:
        ...
        app_verify_dev->mode 				= AL_VERIFY_DEV_MODE_SRV;
        //固定的
      ...
      case FEX_CMD_fes_trans:
        ...
        if( fes_old_data->len ){
            if( fes_old_data->u2.DOU == 2 ){
                ...
                trans_data.act_send_buffer = fes_old_data->addr;	
                //设置发送地址
                trans_data.send_size = fes_old_data->len;
                //设置发送长度
                trans_data.last_err = 0;
                trans_data.app_next_status = SUNXI_USB_EFEX_APPS_SEND_DATA;
            }else{
                //(fes_old_data->u2.DOU == (0 or 1))	
                //下载数据
                #ifdef SUNXI_USB_DEBUG
                uint value;
                value = *(uint *)fes_old_data->addr;
                ...
                trans_data.type = SUNXI_EFEX_DRAM_TAG;
                //写到dram的数据
                trans_data.act_recv_buffer = fes_old_data->addr;	
                //设置接收地址
                trans_data.recv_size = fes_old_data->len;
                //设置接收长度
                trans_data.last_err = 0;
      ...
      {  
        ...
        //printf( "trans_data.type = %x.\n", trans_data.type ); 
        //数据类型:MBR,BOOT1,BOOT0...以及分区类型
        if( (trans->type & SUNXI_EFEX_DRAM_MASK) == SUNXI_EFEX_DRAM_MASK ){
            ...
            if( (SUNXI_EFEX_DRAM_MASK | SUNXI_EFEX_TRANS_FINISH_TAG) == trans->type ){
                ...
                trans_data.act_recv_buffer = (uint)trans_data.base_recv_buffer + trans_data.to_be_recved_size;
                //设置接收地址
            }
            trans_data.recv_size = trans->len;	
            //设置接收长度,字节单位
            ...
            //属于flash数据,分别表示起始扇区,扇区数
            trans_data.act_recv_buffer = (uint)(trans_data.base_recv_buffer + SUNXI_EFEX_RECV_MEM_SIZE/2);	 
            //设置接收地址
            trans_data.recv_size = trans->len;
            //设置接收长度,字节单位
      ...
      case FEX_CMD_fes_up:
      {
        ...
        if( (trans->type & SUNXI_EFEX_DRAM_MASK) == SUNXI_EFEX_DRAM_MASK ){
            ...
            if( (SUNXI_EFEX_DRAM_MASK | SUNXI_EFEX_TRANS_FINISH_TAG) == trans->type ){
                ...
                trans_data.act_send_buffer = (uint)trans_data.base_send_buffer + trans_data.send_size;	 
                //设置发送数据地址
            }
            #endif
            trans_data.act_send_buffer = trans->addr;
            //设置发送地址,属于字节单位
      ...
      {   
        ...
        //FEX_CMD_fes_verify_value, start 0x8000, size high 0x0:low 0x94d400
        //FEX_CMD_fes_verify_value 0xc4b52227
        
        if( cmd_verify->start == 0x8000 ){
            //verify_data->media_crc = 0xc4b52227;
        }
...
{           
    if( data_type == SUNXI_EFEX_MBR_TAG ){
        //传输MBR已经完成
        //检查MBR的正确性
    ...
    }else if(data_type == SUNXI_EFEX_BOOT1_TAG){
        //传输BOOT1已经完成
        printf("SUNXI_EFEX_BOOT1_TAG\n");
        printf("boot1 size = 0x%x\n", trans_data.to_be_recved_size);

        //printf("uboot base_recv_buffer = 0x%x\n", (unsigned int)trans_data.base_recv_buffer);
        //printf("uboot act_recv_buffer = 0x%x\n", trans_data.act_recv_buffer);
        //printf("uboot recv_size = 0x%x\n", trans_data.recv_size);
        //printf("uboot to_be_recved_size = 0x%x\n", trans_data.to_be_recved_size);
        //printf("uboot base_send_buffer = 0x%x\n", (unsigned int)trans_data.base_send_buffer);
        //printf("uboot act_send_buffer = 0x%x\n", trans_data.act_send_buffer);
        //printf("uboot send_size = 0x%x\n", trans_data.send_size);
        //printf("uboot flash_start = 0x%x\n", trans_data.flash_start);
        //printf("uboot flash_sectors = 0x%x\n", trans_data.flash_sectors);
		
        trans_data.last_err = sunxi_sprite_download_uboot( (void *)trans_data.base_recv_buffer, 
                                                                   uboot_spare_head.boot_data.storage_type, 0 );
    }else if( data_type == SUNXI_EFEX_BOOT0_TAG ){
        //传输BOOT0已经完成
        ...
        trans_data.last_err = axp_set_supply_status_byname( pmu_config.pmu_type, pmu_config.vol_name, 
                                                            pmu_config.voltage,  pmu_config.gate );
...
static int sunxi_efex_state_loop(void *buffer)
{
    ...
    //printf( "sunxi_usb_efex_status = %d.\n", sunxi_usb_efex_status );
    switch( sunxi_usb_efex_status ){
      ...
      case SUNXI_USB_EFEX_SETUP:		
        ...
        if( sunxi_usb_efex_app_step == SUNXI_USB_EFEX_APPS_IDLE ){
            ...
            if( cbw->cmd_package.direction == TL_CMD_RECEIVE ){
                //小机端接收数据	
                ...
                //下一阶段接收到的数据是app
                sunxi_usb_efex_status = SUNXI_USB_EFEX_RECEIVE_DATA;
                //传输阶段,下一阶段将接收数据
                sunxi_usb_efex_app_step = SUNXI_USB_EFEX_APPS_CMD;
                //命令阶段,下一阶段接收的是命令
            }else{
                //setup阶段即usb的bulk传输第一阶段,只能接收数据,不能发送
                ...
                return -1;
                  ...
                  sunxi_usb_efex_app_step == SUNXI_USB_EFEX_APPS_RECEIVE_DATA )	{
            //收到的第二阶段,此时开始解析命令
            sunxi_usb_dbg("APPS: SUNXI_USB_EFEX_APPS_DATA\n");
            if( cbw->cmd_package.direction == TL_CMD_RECEIVE ){	
                //如果要接收数据,则事先启动dma开始接收
                sunxi_usb_dbg("APPS: SUNXI_USB_EFEX_APPS_DATA: TL_CMD_RECEIVE\n");
                sunxi_ubuf->request_size = MIN(cbw->data_transfer_len, trans_data.recv_size);
                //接收长度
                //sunxi_usb_dbg("try to receive data 0x%x\n", sunxi_ubuf->request_size);
                sunxi_usb_efex_write_enable = 0;				
                //设置标志
            ...
            sunxi_usb_efex_status = sunxi_usb_efex_app_step & 0xffff;						
            //识别出传输阶段下一阶段状态																					
      ...
      {     
        ...
        if( sunxi_usb_efex_app_step == SUNXI_USB_EFEX_APPS_SEND_DATA ){
            //来自于命令阶段,要求发送数据,下一阶段只能是发送状态(status_t)
      ...
      case SUNXI_USB_EFEX_RECEIVE_DATA:
        ...
        if( sunxi_usb_efex_write_enable == 1 ){
            //数据部分接收完毕
            csw.status = 0;
            //区分出是命令还是数据
            if( sunxi_usb_efex_app_step == SUNXI_USB_EFEX_APPS_CMD ){
                ...
                if( sunxi_ubuf->request_size != CBW_MAX_CMD_SIZE ){
                    //错误的数据,则返回
                ...
                //sunxi_usb_efex_app_step = SUNXI_USB_EFEX_APPS_DATA;	
                //命令阶段,命令接收完成,下一阶段处理数据
                sunxi_usb_efex_app_step = trans_data.app_next_status;
            }else if( sunxi_usb_efex_app_step == SUNXI_USB_EFEX_APPS_RECEIVE_DATA ){
                //来自于命令阶段,要求接收数据,下一阶段只能是发送状态(status_t)
                //表示当次数据已经接收完成
                ...
                sunxi_usb_efex_app_step = SUNXI_USB_EFEX_APPS_STATUS;
                if( trans_data.type & SUNXI_EFEX_DRAM_MASK ){
                    //表示属于内存数据,需要事先保存到内存中
                    sunxi_usb_dbg("SUNXI_EFEX_DRAM_MASK\n");
                    if( trans_data.type & SUNXI_EFEX_TRANS_FINISH_TAG ){
                        //表示当前类型数据已经接收完成
                    ...
                    printf("SUNXI_EFEX_FLASH_MASK\n");
                    ret_value = sunxi_sprite_write( trans_data.flash_start, trans_data.flash_sectors, 
                    ...
                    if( ret_value == 0 ){
                        ...
                        printf( "ret_value = %d.\n", ret_value );                    
                        printf( "sunxi usb efex ok: write flash from 0x%x, sectors 0x%x ok\n", 
                                 trans_data.flash_start, trans_data.flash_sectors );   
                    }
                }
            }
      ...
      {    
        ...
        if( trans_data.app_next_status == SUNXI_USB_EFEX_APPS_SEND_DATA ){
            ...
            sunxi_ubuf->request_size = trans_data.recv_size;   
            //接收长度
            sunxi_usb_efex_write_enable = 0;                
            //设置标志
            if( sunxi_ubuf->request_size ){
      ...
      {  
        ...
        //printf( "wait dma recv finish...\n" );         
		//printf( "sunxi_usb_efex_write_enable = %d.\n", sunxi_usb_efex_write_enable );
