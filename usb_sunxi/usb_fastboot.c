...
static int sunxi_usb_fastboot_status_pre = -1;
...
static int __usb_get_descriptor(struct usb_device_request *req, uchar *buffer)
{
    ...
    switch( req->wValue >> 8 ){
      ...
      {         
        ...
        //IN
        //USB设备的端点地址 
        //Bit 7:   方向,对于控制端点可以忽略.1/0:IN/OUT.
        //Bit 6-4: 保留.
        //BIt 3-0: 端点号.
        ep_in->bmAttributes       = USB_ENDPOINT_XFER_BULK;
        //端点属性.
        //Bit 7-2: 保留.
        //Bit 1-0: 00控制,01同步,02批量,03中断.
      ...
      {  
        ...
        if( string_index == 0 ){
            ...
            printf("fast boot sunxi usb err1: string line %d is not supported\n", string_index);
            /* sent zero packet */
...
//字符串描述符:其中字符串描述符是可选的.
//如果不支持字符串描述符,其设备,配置,接口描述符内的所有字符串描述符索引都必须为０
...
static int sunxi_fastboot_state_loop(void  *buffer)
{
    ...
    if( sunxi_usb_fastboot_status_pre != sunxi_usb_fastboot_status ){
        printf( "sunxi_usb_fastboot_status = %d\n", sunxi_usb_fastboot_status ); 
        sunxi_usb_fastboot_status_pre = sunxi_usb_fastboot_status;
    }
    ...
    switch( sunxi_usb_fastboot_status ){
      ...
      case SUNXI_USB_FASTBOOT_RECEIVE_DATA:
        ...
        if( (fastboot_data_flag == 1) && 
            ((char *)sunxi_ubuf->rx_req_buffer == all_download_bytes + trans_data.base_recv_buffer) ){
            //传输完毕	  		
