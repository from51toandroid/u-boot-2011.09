...
int check_android_misc(void)
{
	...
	//printf( "in check_android_misc.\n");
	//printf( "work_mode = %d.\n", uboot_spare_head.boot_data.work_mode );
	if( uboot_spare_head.boot_data.work_mode != WORK_MODE_BOOT ){
		return 0;
	}

	//printf( "gd->force_shell = %d.\n", gd->force_shell );
	if( gd->force_shell ){
		char delaytime[8];
	...
	macdbg_dmphex( &misc_message->command[0], 32 );

	
	if( (misc_message->command[0] == 0x00) || 
		(misc_message->command[0] == 0xff) ){
		printf( "misc_message->command = %x.\n", misc_message->command[0] );
		if( mode == USER_SELECT_MODE ){                                         
			//说明探测阶段有按键按下
			...
			while(status != POWERON_KEY_PRESSED){
				//图片显示菜单
				status = sunxi_probe_key_pressed();
				if( status == KEY_PRESSED ){
					debug("key_ststus = 0x%x\n ",status);
					user_mode_status_update(status);     	
					//更新菜单的选项
					show_user_select_menu_ui(); 				
					//显示当前模式的ui
				}
			}
	...
	//printf("loglel_change_flag = %x\n", loglel_change_flag);
	//printf("misc_message->command = %s\n", misc_message->command);
	//puts( "find efex cmd1.\n" );
	if( ! loglel_change_flag ){   
		//add by young, if you want to enter debug_mode, so do enter boot_normal
	    //最终统一判断命令
		...
		if( !strcmp(misc_message->command, "boot-resignature") ){
			...
			sunxi_str_replace(boot_commond, "boot_normal", "boot_recovery");
		}else if( !strcmp(misc_message->command, "Global Action Reboot!!") ){
		    if( mode == ANDROID_RECOVERY_MODE ){
			    strcpy(misc_message->command, "boot-recovery");
			    puts("Recovery detected, will dp_mcu recovery\n");
			    sunxi_str_replace(boot_commond, "boot_normal", "boot_recovery");
		    }
		}
	}
