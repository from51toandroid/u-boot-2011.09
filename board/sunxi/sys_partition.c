...
{   
	...
	p = buf;
	printf( "buf addr : [ %08x ].\n", (unsigned int)p );
    for( i=0x00; i<len; i=i+8 ){
		 if( buf[i] || buf[i+1] || buf[i+2] || buf[i+3] || buf[i+4] || buf[i+5] || buf[i+6] || buf[i+7] ){
...
int sunxi_partition_init(void)
{
	...
	//dump_hex( mbr_buf, sizeof(mbr_buf) );                                               
    //macdbg_dmphex( mbr_buf, 0x600 );                                                        
