...
int sprite_uichar_init(int char_size)
{
    ...
    if( open_font( (const char *)font_name, ui_char_info.word_size, 
        sprite_source.screen_width, (uchar *)sprite_source.screen_buf) ){   
        //打开字库
        printf( "ui_char_info.word_size is %d\n", ui_char_info.word_size );
    ...
    ui_char_info.total_height = ((sprite_source.screen_size / 4) / (sprite_source.screen_width)) / (ui_char_info.word_size);   
    //总的高度,可以显示的行数
    ui_char_info.rest_screen_height  = sprite_source.screen_height/(ui_char_info.word_size);  	
    //记录屏幕的剩余高度,行数,剩余1行不用
    ui_char_info.rest_display_height = ui_char_info.total_height;    						
    //记录显示的剩余高度,行数,剩余1行不用
    ui_char_info.rest_screen_width   = sprite_source.screen_width;                        		
    //剩余宽度等于显示宽度,像素单位
...
static int uichar_putchar(__u8 ch)
{
    ...
    width = draw_bmp_ulc(ui_char_info.x, ui_char_info.y, sprite_source.color);    
	//显示字符,返回当前显示字符的宽度,像素单位
    //printf( "ui_char_info.x = %d.\n", ui_char_info.x );
	//printf( "ui_char_info.y = %d.\n", ui_char_info.y );
	//printf( "sprite_source.color = %x.\n", sprite_source.color );
	//printf( "width = %d.\n", width );

	
	ui_char_info.x += width;
    ui_char_info.rest_screen_width -= width;                        
	//记录当前行剩余的像素
    //调用打印函数
	//printf( "ui_char_info.rest_screen_width = %d.\n", ui_char_info.rest_screen_width );
	return ret;
}
/*
**********************************************************************************************************************
...
void sprite_uichar_printf(const char * str, ...)
{
	...
	base_color = SPRITE_CARTOON_GUI_RED;
	base_color &= 0xffffff;
