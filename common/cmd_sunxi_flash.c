...
#include "../nand_sunxi/sun8iw5/nand_interface/nand_bsp.h"
...
extern void dump_hex(char *buf, int len);

int my_do_sunxi_flash(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	int ret = 0;
...
//ffffffffffffffff
	...
	if( strncmp(cmd, "read", 4) == 0 ){
		...
		tick_printf("sunxi flash log_read :offset %x, %x sectors %s\n", start_block, rblock,
		       ret ? "OK" : "ERROR");

        //746f6f62 64616f6c
		p = (ulong *)addr;
        for( i=0x00; i<rblock; i++ ){
             for( j=0x00; j<128; j++ ){
                  if( p[j] != 0x00000000 && p[j] != 0xffffffff ){
                      printf( "i,j,p = %x, %x, %p.\n", i,j,p );
				  }
			 }
			 p = p + 128;
		...
		//sunxi_sprite_phyread
		//macdbg_dmphex(const char * buff,int len)
        #if 1
		memset( (void *)(addr), 0, 0x100000 );
		ret = sunxi_flash_phyread(start_block, rblock, (void *)addr);
		tick_printf("sunxi flash phy_read :offset %x, %x sectors %s\n", start_block, rblock, ret ? "OK" : "ERROR" );
        return ret == 0 ? 0 : 1;
		#endif
		...
		memset( (void *)(addr), 0, 0x100000 );
		para.chip  = 0;
		para.block = start_block;
		para.page  = rblock;
		//1page  = 16k = 32ÉÈÇø
		//1ÉÈÇø  = 512byte
		//1block = 4M byte = 256 page
		para.mainbuf = (void *) (addr);
		para.oobbuf = oob_buf;
		...
		printf( "ret = %d.\n", ret );
		//dump_hex(para.mainbuf, 0x100000);
...
U_BOOT_CMD(
	sunxi_flash, CONFIG_SYS_MAXARGS, 1, my_do_sunxi_flash,
