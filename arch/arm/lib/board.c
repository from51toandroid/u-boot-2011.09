...
void board_init_f(ulong bootflag)
{
	...
	my_msg( "hello uart v1.01\r\n" );                                                
	#endif                                    
                                                                         
    //my_msg( "hello uart v1.07\r\n" );  
	//gd->uart_console = 2;
    printf( "gd->uart_console = %x.\n", gd->uart_console );
 ...
 * running from RAM and have a "normal" C environment, i. e. global
   在RAM中运行了(start.S中进行了重定位)并且有正常的c运行环境,也就是说全局
 * data can be written, BSS has been cleared, the stack size in not
   数据可以写入,BSS已经清除了,堆栈不再是
 * that critical any more, etc.
   临时的(临时堆栈使用A33 cpu内部的内存,在代码重定位之前,跑在ROM中的时候)
 ************************************************************************
 */
//board_init_f
...
{    
    ...
    printf("in board_init_r.\n");
    printf( "current pc.1 = %x.\n", get_current_pc() );
    ...
    if( flash_size > 0 ){
        ...
        if( s && (*s == 'y') ){
            printf( "CRC: %08X", crc32(0,(const unsigned char *)CONFIG_SYS_FLASH_BASE,
                    flash_size) );
        }
        putc('\n');
        #else
	...
	if( workmode == WORK_MODE_BOOT ){
		...
		if( !ret ){
            ...
            if( !gd->chargemode ){
				...
				//extern int sprite_cartoon_test(void);
	            //sprite_cartoon_test();
            }
		}
        #endif
