...
static int sunxi_null_op(unsigned int start_block, unsigned int nblock, void *buffer)
{
	return 0;
}

static int sunxi_null_erase(int erase, void *mbr_buffer)
{
	return 0;
}

static uint sunxi_null_size(void)
{
	return 0;
}

static int sunxi_null_init(int flag)
{
	return -1;
}

static int sunxi_null_exit(int force)
{
	return -1;
}

static int sunxi_null_flush(void)
{
	return 0;
}
...
sunxi_flash_nand_read(uint start_block, uint nblock, void *buffer)
{
    ...
    printf("in sunxi_flash_nand_read.\n" );
...
sunxi_flash_mmc_read(unsigned int start_block, unsigned int nblock, void *buffer)
{
    ...
    //printf("in sunxi_flash_mmc_read.\n" );

	//printf("mmc_boot->block_dev.block_read %p.\n", mmc_boot->block_dev.block_read ); 
...
int sunxi_flash_read (uint start_block, uint nblock, void *buffer)
{
	...
	//printf("sunxi flash read : start %d, sector %d\n", start_block, nblock);
	//printf("sunxi_flash_read_pt %p.\n", sunxi_flash_read_pt );                    
	return sunxi_flash_read_pt(start_block, nblock, buffer);   
	//sunxi_flash_mmc_read
}                                                                 
//sunxi_flash_mmc_read    
//sunxi_flash_nand_read
...
int sunxi_flash_phyread (uint start_block, uint nblock, void *buffer)
{
    ...
    printf( "in sunxi_flash_phyread.\n");
	printf( "sunxi_flash_phyread_pt = %p.\n", sunxi_flash_phyread_pt );
...
{     
    uint offset;
    int ret;
    //printf( "sunxi_flash_part_read = %p\n", sunxi_flash_part_read );
